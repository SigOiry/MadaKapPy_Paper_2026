...
Across the lagoon, the daily growth rate (DGR) of cultivation plots displayed a marked spatial structure (@fig-plot-productivity-map). Higher mean DGR values were generally observed in plots located along the eastern side of the lagoon, closer to the coral barrier, where several plots reached ca. 4 %.day⁻¹. In contrast, plots positioned toward the central channel and the inner lagoon showed consistently lower DGR values, in some cases close to 2 %.day⁻¹. This pattern was consistent across the entire lagoon, suggesting that DGR may be influenced by a dominant environmental gradient across the lagoon. Within several modules, DGR also varied among adjacent plots, indicating substantial fine-scale heterogeneity. The main exception to the overall spatial gradient was the northernmost cultivation module, where low DGR values were observed across most plots, irrespective of their relative position within the module.
...
#| warning: false
#| message: false
#| fig-cap: "Spatial variability in the Daily Growth Rate (annual average) of seaweed cultivation plots in 2025. Each polygons represent a 40*10m cultivation plot. The insert shows one block of five farms (each farm is composed of six cultivation plots)."
#| out-width: "100%"

knitr::include_graphics("Figures/Map_Production/plot_productivity_map_2025-02.png")
```

Cultivation plots were harvested at an average biomass of ca. 2900 kg (@fig-harvested-biomass-distribution A). Depending on plot-specific daily growth rates (DGR), this target biomass was generally reached after 4 to 6 weeks of cultivation. However, there was a large variability of harvested biomass ranging form 1000 to 5000 kg per cultivation plot.

Temporal patterns in DGR differed according to the position of cultivation plots within the lagoon (@fig-harvested-biomass-distribution B). Plots located closest to the reef barrier showed relatively stable DGR throughout the year, with an average of 3.57 %.day⁻¹ in 2025. In these plots, DGR was higher in October, reaching 3.88 %.day⁻¹. In contrast, plots located closest to the channel displayed a stronger seasonal variability. Their maximum DGR was observed in April, when values were comparable to those recorded near the coral reef barrier. However, between June and February, DGR in channel-adjacent plots was significantly lower than in reef-adjacent plots, reaching values as low as 2.25 %.day⁻¹ in December and January.
...
#| warning: false
#| message: false
#| fig-cap: "Harvested biomass of cultivation plot in 2025. (A) Frequency distribution of harvested biomass. (B) Seasonal variability of the Daily Growth Rate (DGR) of harvested plots in 2025. Colors distinguish Est and West plot positions, solid curves show generalized additive model (GAM) trends fitted with cubic regression splines, and shaded envelopes indicate 95% confidence intervals."
#| out-width: "100%"
...
#| warning: false
#| message: false
#| fig-cap: "RGB drone orthomosaics (A). Random Forest classification results for a cultivation plot (B). Brown polygons represent pixels classfied as Kappaphycus while grey polygons represent bottom pixels."
#| out-width: "100%"
...
figure_dir <- file.path("Figures", "Biomass")
dir.create(figure_dir, recursive = TRUE, showWarnings = FALSE)
biomass_fig_png_path <- file.path(figure_dir, "biomass_model_performance.png")
biomass_fig_svg_path <- file.path(figure_dir, "biomass_model_performance.svg")
...
py_run_string(paste(
  ...
  "import numpy as np",
  "",
  "def predict_biomass_h5(model_path, X):",
  "    X = np.asarray(X, dtype=np.float32)",
  "    with h5py.File(model_path, 'r') as f:",
  "        root = f['model_weights']",
  "        w1 = root['dense']['sequential']['dense']['kernel'][()]",
  "        b1 = root['dense']['sequential']['dense']['bias'][()]",
  "        w2 = root['dense_1']['sequential']['dense_1']['kernel'][()]",
  "        b2 = root['dense_1']['sequential']['dense_1']['bias'][()]",
  "        w3 = root['dense_2']['sequential']['dense_2']['kernel'][()]",
  "        b3 = root['dense_2']['sequential']['dense_2']['bias'][()]",
...
model_path <- normalizePath("Data/Model/Biomass_NN_3000Epoch.h5", winslash = "/", mustWork = TRUE)

df <- read_xlsx(harvest_path) %>%
  mutate(
    HarvestingDate = as.Date(HarvestingDate),
    Farm_chr = str_pad(as.character(FarmID), width = 3, side = "left", pad = "0"),
    plot_id = paste0(Farm_chr, "_", ModuleNumber)
  ) %>%
  filter(
    HarvestingDate >= as.Date("2025-12-01"),
    HarvestingDate <= as.Date("2025-12-05")
  ) %>%
  select(plot_id, Biomass = Harvested_Weight)

cadaster <- read_sf("Data/shp/Cadaster_NBS_Dec2025.shp") %>%
  rename(plot_id = ID) %>%
  select(plot_id)

classified_area <- read_sf("Data/Multivariate_Analysis/RF_Morphotypes_classification.shp") %>%
  st_cast("POLYGON", warn = FALSE) %>%
  group_by(plot_id, rf_class) %>%
  reframe(area_rf = sum(st_area(geometry))) %>%
  st_drop_geometry()

prod_2025 <- read_xlsx(harvest_path) %>%
  mutate(
    FarmID = sprintf("%03d", FarmID),
    Module_ID = paste(FarmID, ModuleNumber, sep = "_"),
    Growth_per_day = (Harvested_Weight - 700) / CultureDuration_days / LinesCount
  ) %>%
  select(Module_ID, Growth_per_day) %>%
  group_by(Module_ID) %>%
  reframe(
...
cadaster_filtered <- cadaster %>%
  filter(plot_id %in% unique(classified_area$plot_id)) %>%
  mutate(area = st_area(geometry)) %>%
  st_drop_geometry()

percent_cover <- classified_area %>%
  left_join(cadaster_filtered, by = "plot_id") %>%
  group_by(plot_id) %>%
  reframe(area_rf = sum(area_rf), area = unique(area)) %>%
  mutate(
    area_rf = as.numeric(area_rf),
  ...
  filter(if_all(c(Biomass, area_rf, pc, med, sd), ~ is.finite(.x)))

biomass_model_df <- percent_cover %>%
  transmute(
    plot_id,
    Biomass,
    area_rf,
...
# Rebuild the feature standardization used outside the saved Keras model.
feature_matrix <- as.matrix(biomass_model_df[, c("area_rf", "pc", "med", "sd")])
mu <- colMeans(feature_matrix)
sig <- apply(feature_matrix, 2, sd)
sig[sig == 0] <- 1
x_scaled <- sweep(sweep(feature_matrix, 2, mu, "-"), 2, sig, "/")

plot_df <- biomass_model_df %>%
  mutate(
    pred = exp(as.numeric(py_to_r(py$predict_biomass_h5(model_path, r_to_py(x_scaled)))))
  )

r2 <- cor(plot_df$Biomass, plot_df$pred, use = "complete.obs")^2
rmse <- sqrt(mean((plot_df$Biomass - plot_df$pred)^2))
mae <- mean(abs(plot_df$Biomass - plot_df$pred))

axis_limits <- range(c(plot_df$Biomass, plot_df$pred), na.rm = TRUE)
axis_padding <- diff(axis_limits) * 0.05
axis_limits <- c(axis_limits[1] - axis_padding, axis_limits[2] + axis_padding)

stats_df <- tibble(
  x = axis_limits[1] + diff(axis_limits) * 0.04,
  y = axis_limits[2] - diff(axis_limits) * 0.04,
  label = paste0(
    "n = ", nrow(plot_df),
    "\nR² = ", sprintf("%.2f", r2),
    "\nRMSE = ", comma(round(rmse)),
    "\nMAE = ", comma(round(mae))
  )
)

performance_plot <- ggplot(plot_df, aes(x = Biomass, y = pred)) +
  geom_abline(
    slope = 1,
    intercept = 0,
    linetype = "22",
  ...
  scale_fill_gradientn(
    colours = c("#0B3C49", "#177E89", "#49A078", "#F4C95D"),
    name = "Plot cover (%)",
  ...
  theme(
    ...
    panel.grid.major = element_line(colour = "#D7E2E7", linewidth = 0.3),
    axis.title = element_text(face = "bold", colour = "#1F2F38"),
    axis.text = element_text(colour = "#304651"),
    legend.position = "bottom",
    legend.title = element_text(face = "bold", colour = "#1F2F38"),
    legend.text = element_text(colour = "#304651"),
    plot.background = element_rect(fill = "white", colour = NA),
    panel.background = element_rect(fill = "white", colour = NA)
  )

ggsave(
  biomass_fig_png_path,
  performance_plot,
  width = 180,
  height = 160,
  units = "mm",
...
The biomass model showed a strong agreement between observed and predicted harvested biomass (n = 84; @fig-biomass-performance). Predicted biomass values were closely distributed around the 1:1 line, indicating that the model captured most of the variability in harvested biomass among cultivation plots. Model performance was high, with an R² of 0.88, an RMSE of 253 kg, and an MAE of 186 kg. Most predictions were concentrated between 2000 and 4000 kg, corresponding to the main range of observed harvested biomass.

Despite the overall good agreement, some deviations from the 1:1 relationship were observed. The fitted regression line showed a slight tendency to overestimate biomass at the lower end of the observed range and to underestimate biomass for the highest observed values. This pattern suggests that model uncertainty increased for plots in edge cases of harvested biomass, which may be related to the lower number of training samples in these ranges or to specific characteristics of these plots that were not fully captured by the model features.

```{r fig-biomass-performance}
#| echo: false
#| fig-cap: "Observed versus predicted biomass produced by the neural network used in the biomass estimation of MadaKapPy. The dashed line shows the 1:1 relationship, and the solid line shows the fitted trend."
#| out-width: "88%"

knitr::include_graphics("Figures/Biomass/biomass_model_performance-01.png")
```
