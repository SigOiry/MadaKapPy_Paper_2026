...
filtered_df["harvest_month"] = (
    filtered_df["HarvestingDate"].dt.to_period("M").dt.to_timestamp()
)
...
filtered_df["culture_duration_days"] = harvest_df.loc[
    filtered_df.index, "CultureDuration_days"
].astype(float)

temporal_df = filtered_df.loc[
    (filtered_df["harvested_biomass_kg"] >= 1000)
    & filtered_df["culture_duration_days"].notna()
    & (filtered_df["culture_duration_days"] > 0)
].copy()
if temporal_df.empty:
    raise ValueError("No temporal-series values remain after excluding biomass below 1000 kg.")
temporal_df["growth_rate_kg_day"] = (
    temporal_df["harvested_biomass_kg"] / temporal_df["culture_duration_days"]
)
temporal_df["harvest_date"] = pd.to_datetime(temporal_df["HarvestingDate"])


def fit_penalized_spline_gam(x, y, x_pred, degree=3, n_segments=6, penalty=25.0):
    x = np.asarray(x, dtype=float)
    y = np.asarray(y, dtype=float)
    x_pred = np.asarray(x_pred, dtype=float)
    x_min_local = float(np.min(x))
    x_max_local = float(np.max(x))

    internal_knots = np.linspace(x_min_local, x_max_local, n_segments + 1)[1:-1]
    knot_vector = np.concatenate(
        [
            np.repeat(x_min_local, degree + 1),
            internal_knots,
            np.repeat(x_max_local, degree + 1),
        ]
    )
    n_basis = len(knot_vector) - degree - 1
    identity = np.eye(n_basis)
    basis_functions = [
        BSpline(knot_vector, identity[i], degree, extrapolate=True)
        for i in range(n_basis)
    ]

    design_matrix = np.column_stack([basis(x) for basis in basis_functions])
    prediction_matrix = np.column_stack([basis(x_pred) for basis in basis_functions])
    difference_matrix = np.diff(np.eye(n_basis), n=2, axis=0)
    penalty_matrix = penalty * (difference_matrix.T @ difference_matrix)

    coefficients = np.linalg.solve(
        design_matrix.T @ design_matrix + penalty_matrix,
        design_matrix.T @ y,
    )
    return prediction_matrix @ coefficients
...
if harvested_biomass_kg.size > 1 and np.nanstd(harvested_biomass_kg) > 0:
    ...
    ax_hist.plot(x_kde, y_kde, color="#6b7280", linewidth=1.4, alpha=0.95)

ax_hist.set_xlabel("Harvested biomass (kg)", color="#222222")
ax_hist.set_ylabel("Frequency", color="#222222")
...
ax_box.set_xlabel("Harvesting date", color="#222222")
ax_box.set_ylabel(r"Growth rate (kg day$^{-1}$)", color="#222222")
growth_max = float(np.nanmax(temporal_df["growth_rate_kg_day"]))
growth_min = float(np.nanmin(temporal_df["growth_rate_kg_day"]))
growth_tick_step = 10 if growth_max <= 120 else 20
growth_ymin = max(0, np.floor(growth_min / growth_tick_step) * growth_tick_step)
growth_ymax = np.ceil(growth_max / growth_tick_step) * growth_tick_step
ax_box.set_ylim(growth_ymin, growth_ymax)
ax_box.set_yticks(np.arange(growth_ymin, growth_ymax + growth_tick_step, growth_tick_step))
ax_box.margins(x=0.01)

x_points = mdates.date2num(temporal_df["harvest_date"])
y_points = temporal_df["growth_rate_kg_day"].to_numpy(dtype=float)
ax_box.scatter(
    temporal_df["harvest_date"],
    y_points,
    s=10,
    facecolor="#9da8b3",
    edgecolor="none",
...
x_pred = np.linspace(x_points.min(), x_points.max(), 400)
y_gam = fit_penalized_spline_gam(
    x_points,
    y_points,
    x_pred,
    degree=3,
...
ax_box.set_xlim(date_min - pd.Timedelta(days=4), date_max + pd.Timedelta(days=4))
ax_box.xaxis.set_major_locator(mdates.MonthLocator())
ax_box.xaxis.set_major_formatter(mdates.DateFormatter("%b"))
...
#| warning: false
#| message: false
#| fig-cap: "Harvested biomass of cultivation modules in 2025. (A) Frequency distribution of harvested biomass. (B) Growth rate, computed as harvested biomass divided by cultivation duration (days) along the year 2025. Individual points represent harvested plots and the smooth curve shows a GAM temporal trend."
#| out-width: "100%"

knitr::include_graphics("Figures/harvested_biomass_distribution_temporal_variability.png")
```
